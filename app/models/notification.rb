class Notification < ApplicationRecord
  ...
  def message
    if subjectable_type == 'Follow'
      ...
      I18n.t('notifications.reaction', user: subjectable.user.name, target: subjectable.comment.post.truncated(16))
    elsif subjectable_type.in?(['CheerReaction'])
      I18n.t('notifications.reaction', user: subjectable.user.name, target: subjectable.cheer.post.truncated(16))
    elsif subjectable_type == 'Pin'
